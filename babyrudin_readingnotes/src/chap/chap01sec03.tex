...
\begin{myRemark}
    ...
    \begin{enumerate}[(a)]
        ...
        \item The field axioms clearly hold in $\mathbb{Q}$, the set of all rational numbers, if addition and multiplication have their customary meaning. Thus $\mathbb{Q}$ is a field.
        \item Although it is not our purpose to study fields (or any other algebraic structures) in detail, it is worthwhile to prove that some familiar properties of $\mathbb{Q}$ are consequences of the field axioms; once we do this, we will \underline{not need to do it} again for the real numbers and for the complex numbers.
    \end{enumerate}
\end{myRemark}
...
\begin{proof}
    \begin{asparaenum}[(a)]
        ...
        \end{align*}
        \item (a)取  $z=1$. $y=z=1$.
        \item (a)取  $z=\frac{1}{x}$. $y=z=\frac{1}{x}$.
        \item (c)取  $x=\frac{1}{x'}$. $y=1/(1/x')$.
    \end{asparaenum}
\end{proof}
...
\mybox{
    有序域$F$也是有序集, 
    由于有理数域 $\mathbb{Q}$, 实数域 $\R{}$ 都是有序域, 
    这里使用有理数域 $\mathbb{Q}$ 证明的有序集的性质也可以直接用于实数域 $\R{}$.}
