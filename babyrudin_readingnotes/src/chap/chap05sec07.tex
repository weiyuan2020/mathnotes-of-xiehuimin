...
\begin{myRemark}
    Definition \ref{def:5.1} applies without any change to complex functions $f$ defined on $[a, b]$, 
...
we may still apply Definition \ref{def:5.1} to define $\mathbf{f}'(x)$. 
...
if $fg$ is replaced by the inner product $\mathbf{f} \cdot \mathbf{g}$ (see Definition \ref{def:4.3}).
