...
It's important to observe that every subset $Y$ of metric space $X$ is a metric space in its own right, with the same distance function. For it is clear that if conditions (a) to (c) of Definition \ref{def:2.15} hold for $p, q, r \in X$, they also hold if we restrict $p, q, r$ to lie in $Y$.
...
\begin{proof}
    \begin{enumerate}[(a)]
        ...
        \item If $E = \bar{E}$, (a) implies that $E$ is closed. If $E$is closed, then $E' \subset E$ [by Definitions \ref{def:2.18}(d) and \ref{def:2.26}], hence $\bar{E} = E$.
