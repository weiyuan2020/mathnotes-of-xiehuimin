...
\begin{thm}
    \label{thm:2.38}
    If $\{I_n\}$ is a sequence of intervals in $\mathbb{R}^1$, 
...
% \myproof{
%     Let I be a k-cell, consisting of all points x = (x1, ..• , xk) such
...
% I=> 11 => 12 => /3 => · • • ;
% (b)
% In is not covered by any finite subcollection of {Ga};
% (c) ifxe/nandye/n, then lx-yl ~2-nb.
% By (a) and Theorem 2.39, there is a point x* which lies in every In.
% For some tx, x* e Ga.
% Since Ga is open, there exists r > 0 such that
...
% \myproof{
%     If (a) holds, then E c / for some k-cell /, and (b) follows from
...
% If E is not closed, then there is a point x 0 e Rk which is a limit point
% of E but not a point of E. For n = 1, 2, 3, ... , there are points xn e E
% such that I xn -
% x0 I < 1/n. Let S be the set of these points xn. Then Sis
